package arrays;

import java.util.Arrays;
import java.util.Scanner;

public class DuplicateNumberInArray {

	public static void main(String[] args) {
		
	
		//ODO Auto-generated method stub
		
		int[] integer = {2,2,3,4,5,5,6,4};
	//nterheNumber=0;
		//Scanner sc = new Scanner(System.in);
		
	
		
		for(int i=0;i<integer.length;i++)
			
		{
		//	System.out.println("enter the number");
		//	enterTheNumber = sc.nextInt();
		//	integer[i] = enterTheNumber;
		
	}//	System.out.println("entered number  = " + Arrays.toString(integer));
	
		int[] duplicate =new int[integer.length];
		int count=0;
		for(int i=0;i<integer.length;i++)
			for (int j =i+1;j<integer.length;j++)
			{if(integer[i]==integer[j])
			
			{duplicate[count]= integer[i];
			System.out.println(duplicate[count]);
			count++;
				
			}
			
} System.out.println(" duplicate  number "+ Arrays.toString(duplicate));	
}

		
	/*	 int arr[] = { 50, 0,10, 40, 20, 10, 10, 60, 30, 70,0,70,40};
	      
	      // create another array of similar size
	      int temp[] = new int[arr.length];
	      int count = 0;
	      
	      // traverse original array
	      for(int i=0; i<arr.length; i++) {
	         
	         int element = arr[i];
	         boolean flag = false;
	         
	         // check current element is already 
	         // checked or not
	         for(int j=0; j<count; j++) {
	            if(temp[j] == element) {
	               flag = true;
	               break;
	            }
	         }
	         
	         // if already exist then don't check
	         if(flag) {
	            continue;
	         }
	         
	         // check occurrence of element
	         for(int j=i+1; j<arr.length; j++) {
	            if(arr[j] == element) {
	               temp[count++] = element;
	               // found, therefore break
	               break;
	            }
	         }
	      }
	      
	      // display total repeated elements
	      System.out.println("Total Repeated elements: " + count);
	      // display repeated elements
	      System.out.println("Repeated elements are: ");
	      for (int i = 0; i < count; i++) {
	         System.out.print(temp[i]+" ");*/
	      }    
	   
	